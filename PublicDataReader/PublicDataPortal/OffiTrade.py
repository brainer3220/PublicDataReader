...
class OffiTradeReader(Common):

    def __init__(self, serviceKey):
        super().__init__(serviceKey)
        # ServiceKey 유효성 검사
        api_url = "http://openapi.molit.go.kr/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcOffiTrade?serviceKey=" + self.serviceKey
        super().test(api_url)
    ...
    def DataReader(self, LAWD_CD, DEAL_YMD):
        '''
        지역코드와 계약월을 입력받고, 오피스텔 매매 신고 정보를 Pandas DataFrame 형식으로 출력합니다.
        '''

        # URL
        url_1 = "http://openapi.molit.go.kr/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcOffiTrade?LAWD_CD="+LAWD_CD
